...
#############################
# THE BIG PICTURE
# This is a "brute force" implementation of a direct pathway model of how the
# basal ganglia performs category learning. I've implemented this and several
# other crude versions of your final project models in this format because a few
# students reported having trouble following the numpy array manipulations in
# the more elegant implementations. It's up to you how you implement your final
# project. Neither form will be penalized or rewarded.
#############################
...
for trl in range(n_trl - 1):
    ...
    for i in range(1, n):  # iterate over time steps
        ...
        dvdt_pm_A = (k * (v_pm_A[i - 1] - vr) * (v_pm_A[i - 1] - vt) - u_pm_A[i - 1] + E + w_thl_pm * g_thl_A[i-1]) / C
        ...
        # pm B
        C, vr, vt, vpeak, a, b, c, d, k, E =  [100, -60, -40, 35, 0.03, -2, -50, 100, 0.7, 0]
        dvdt_pm_B = (k * (v_pm_B[i - 1] - vr) * (v_pm_B[i - 1] - vt) - u_pm_B[i - 1] + E + w_thl_pm * g_thl_B[i-1]) / C
    ...
    # response selection at each time step.
    #############################
    ...
    # We begin by computing the post-synaptic activity component 
    # of ctx-msn synapses (i.e., of the MSN neuron activity). The
    # Pre-synaptic activity is the visual cortex activity (vis_act)
    # and will be computed below inside some nested for loops. 
    ...
    for ii in range(vis_dim):
        for jj in range(vis_dim):
            ...
            if rpe[trl] > 0:
                ...
                dw_A = beta_w * pre * post_ltp_1_A * post_ltp_2_A * (1 - w_vis_msn_A[ii, jj]) * rpe[trl]
                dw_B = beta_w * pre * post_ltp_1_B * post_ltp_2_B * (1 - w_vis_msn_B[ii, jj]) * rpe[trl]

            # NOTE: LTD is also caused by weak post-synaptic activity, but for simplicity I have not
            # included that here. Again, you should strive to replicate what is done in the paper you
            # are assigned.
