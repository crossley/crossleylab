...
const validArcs = new Set(['conductance_models']);
const validStatuses = new Set(['active', 'draft', 'rebuild', 'future', 'archived']);
...
for (const [idx, lesson] of lessons.entries()) {
  ...
  for (const key of ['id', 'title', 'description', 'htmlPath', 'tsEntry', 'arc', 'status']) {
  ...
  if (typeof lesson.arc !== 'string' || !validArcs.has(lesson.arc)) {
    errors.push(`Invalid arc at ${label}: ${String(lesson.arc)}`);
  }
