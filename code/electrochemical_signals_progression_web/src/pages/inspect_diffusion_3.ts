...
function simulateChannel(
  params: { T: number; dt: number; numParticles: number; boxWidth: number; boxHeight: number; wallThickness: number; diffusionSd: number; channelYMin: number; channelYMax: number; },
  seed: number
): Trajectory {
  ...
  return { x, y, frames, numParticles, dt, boxWidth, boxHeight, leftWall, rightWall, channelYMin, channelYMax };
}
...
function worldToCanvas(x: number, y: number, traj: Trajectory, w: number, h: number): [number, number] {
  const px = ((x + traj.boxWidth / 2) / traj.boxWidth) * w;
  const py = h - ((y + traj.boxHeight / 2) / traj.boxHeight) * h;
  return [px, py];
}

function drawTrajectory(canvas: HTMLCanvasElement, traj: Trajectory, frame: number, pointSize: number, title: string): void {
  ...
  const [, yTop] = worldToCanvas(0, traj.channelYMax, traj, w, h);
  const [, yBottom] = worldToCanvas(0, traj.channelYMin, traj, w, h);
  ctx.fillStyle = 'rgba(200, 220, 255, 0.08)';
  ctx.fillRect(wl, 0, wr - wl, h);
  ctx.fillStyle = 'rgba(159, 255, 106, 0.12)';
  ctx.fillRect(wl, yTop, wr - wl, yBottom - yTop);
...
const narrowRangeLabel = getEl<HTMLElement>('#narrow-range-label');
const wideRangeLabel = getEl<HTMLElement>('#wide-range-label');

const inputs = {
  numParticles: getEl<HTMLInputElement>('#num-particles'),
  diffusionSd: getEl<HTMLInputElement>('#diffusion-sd'),
  narrowYMin: getEl<HTMLInputElement>('#narrow-y-min'),
  narrowYMax: getEl<HTMLInputElement>('#narrow-y-max'),
  wideYMin: getEl<HTMLInputElement>('#wide-y-min'),
  wideYMax: getEl<HTMLInputElement>('#wide-y-max'),
...
let narrowTraj = simulateChannel({ ...defaultSim, channelYMin: defaultSim.narrowYMin, channelYMax: defaultSim.narrowYMax }, currentSeed);
let wideTraj = simulateChannel({ ...defaultSim, channelYMin: defaultSim.wideYMin, channelYMax: defaultSim.wideYMax }, currentSeed);
let isPlaying = true;
let playhead = 0;
...
function writeInputs(): void {
  ...
  setNumberInput(inputs.narrowYMin, simParams.narrowYMin, 2);
  setNumberInput(inputs.narrowYMax, simParams.narrowYMax, 2);
  setNumberInput(inputs.wideYMin, simParams.wideYMin, 2);
  setNumberInput(inputs.wideYMax, simParams.wideYMax, 2);
...
function readSimInputs(): SimParams {
  ...
  return {
    ...
    narrowYMin: clamp(Number(inputs.narrowYMin.value) || 0, -boxHeight / 2, boxHeight / 2),
    narrowYMax: clamp(Number(inputs.narrowYMax.value) || 0, -boxHeight / 2, boxHeight / 2),
    wideYMin: clamp(Number(inputs.wideYMin.value) || 0, -boxHeight / 2, boxHeight / 2),
    wideYMax: clamp(Number(inputs.wideYMax.value) || 0, -boxHeight / 2, boxHeight / 2)
  };
}
...
function rerunFromInputs(rewind = true): void {
  ...
  narrowTraj = simulateChannel({ ...simParams, channelYMin: simParams.narrowYMin, channelYMax: simParams.narrowYMax }, currentSeed);
  wideTraj = simulateChannel({ ...simParams, channelYMin: simParams.wideYMin, channelYMax: simParams.wideYMax }, currentSeed);

  simParams = {
    ...simParams,
    boxWidth: narrowTraj.boxWidth,
    boxHeight: narrowTraj.boxHeight,
    wallThickness: narrowTraj.rightWall - narrowTraj.leftWall,
    narrowYMin: narrowTraj.channelYMin,
    narrowYMax: narrowTraj.channelYMax,
    wideYMin: wideTraj.channelYMin,
    wideYMax: wideTraj.channelYMax
  };
...
function updateRangeLabels(): void {
  narrowRangeLabel.textContent = `[${simParams.narrowYMin.toFixed(1)}, ${simParams.narrowYMax.toFixed(1)}]`;
  wideRangeLabel.textContent = `[${simParams.wideYMin.toFixed(1)}, ${simParams.wideYMax.toFixed(1)}]`;
}

function updateEquationText(): void {
  const stepSd = simParams.diffusionSd * simParams.dt;
  const narrowWidth = simParams.narrowYMax - simParams.narrowYMin;
  const wideWidth = simParams.wideYMax - simParams.wideYMin;
  equationBlock.innerHTML = [
    '<span class="accent">Shared Euler diffusion update (both panels)</span>',
    'x_new = x_old + dxdt · dt',
    'y_new = clamp(y_old + dydt · dt, -H/2, H/2)',
    'dxdt, dydt ~ Normal(0, diffusionSd²)',
    '',
    '<span class="accent-2">Different only in channel gating window</span>',
    'Narrow: y_old ∈ [narrow_y_min, narrow_y_max] permits wall crossing',
    'Wide:   y_old ∈ [wide_y_min,   wide_y_max] permits wall crossing',
    'Otherwise x_new := x_old',
    '',
    `Per-step displacement SD = diffusionSd × dt = ${stepSd.toFixed(3)}`,
    `Channel widths: narrow = ${narrowWidth.toFixed(1)}, wide = ${wideWidth.toFixed(1)}`
  ].join('\n');
}

function updateStatus(frameIndex: number): void {
  const n = sideCounts(narrowTraj, frameIndex);
  const w = sideCounts(wideTraj, frameIndex);
  statusEls.frame.textContent = `${frameIndex + 1}`;
  statusEls.time.textContent = (frameIndex * narrowTraj.dt).toFixed(1);
  statusEls.narrowLeft.textContent = `${n.left}`;
  statusEls.narrowRight.textContent = `${n.right}`;
  statusEls.wideLeft.textContent = `${w.left}`;
  statusEls.wideRight.textContent = `${w.right}`;
  statusEls.dt.textContent = narrowTraj.dt.toFixed(2);
  statusEls.stepSd.textContent = (simParams.diffusionSd * simParams.dt).toFixed(3);
  statusEls.seed.textContent = `${currentSeed >>> 0}`;
  statusEls.frames.textContent = `${narrowTraj.frames}`;
}

function drawCurrentFrame(frameIndex: number): void {
  drawTrajectory(narrowCanvas, narrowTraj, frameIndex, displayParams.pointSize, 'Narrow Channel');
  drawTrajectory(wideCanvas, wideTraj, frameIndex, displayParams.pointSize, 'Wide Channel');
}
...
window.addEventListener('resize', () => {
  drawCurrentFrame(Math.floor(playhead));
});

function animate(ts: number): void {
  const dtSec = (ts - lastTs) / 1000;
  lastTs = ts;
  if (isPlaying && narrowTraj.frames > 0) {
    playhead += dtSec * displayParams.targetFps * displayParams.playbackSpeed;
    if (playhead >= narrowTraj.frames) playhead %= narrowTraj.frames;
  }
  const frameIndex = clamp(Math.floor(playhead), 0, Math.max(0, narrowTraj.frames - 1));
