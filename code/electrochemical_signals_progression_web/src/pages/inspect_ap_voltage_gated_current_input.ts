...
function stepState(state: LiveState, params: SimParams, rng: Rng, pumpEnabled: boolean): void {
  ...
  const targetField = params.electricStrength + params.chargeSeparationGain * chargeImbalanceFrac - params.externalCurrentInput;
...
function drawVmTrace(canvas: HTMLCanvasElement, trace: TraceHistory, currentTime: number, traceWindowMs: number): void {
  ...
  ctx.strokeRect(padL, padT, plotW, plotH);
  ctx.beginPath();
  ctx.moveTo(padL, yVm(0));
  ctx.lineTo(padL + plotW, yVm(0));
  ctx.stroke();
  ...
  <div class="site-shell">
    ...
    <header class="page-head">
      ...
      <ul class="key-points">
        ...
        <li>Field effects still combine fixed-anion, charge-separation, and stimulus terms.</li>
      </ul>
    </header>
