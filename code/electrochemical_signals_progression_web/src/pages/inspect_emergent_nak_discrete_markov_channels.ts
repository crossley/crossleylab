...
function stepState(state: LiveState, params: SimParams, rng: Rng, pumpEnabled: boolean): void {
  ...
  const driftFromFixedAnionLayer = (x: number, y: number, charge: number): number => {
    let driftX = 0;
    const perSourceStrength = params.electricStrength / Math.max(1, state.fixedCount);
...
function drawTrace(canvas: HTMLCanvasElement, trace: TraceHistory, currentTime: number, traceWindowMs: number): void {
  ...
  drawSeries(trace.naOpenFrac, 'rgba(66,200,255,0.45)', true);
  drawSeries(trace.kOpenFrac, 'rgba(245,178,72,0.45)', true);
