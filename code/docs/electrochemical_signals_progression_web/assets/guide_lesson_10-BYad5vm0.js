...
`;n(document.querySelector("#theme-toggle"));document.querySelectorAll(".code-block-wrap").forEach(s=>{const t=s.querySelector("pre");if(!t)return;const e=document.createElement("button");e.className="copy-btn",e.textContent="Copy",e.addEventListener("click",()=>{navigator.clipboard.writeText(t.innerText).then(()=>{e.textContent="Copied!",setTimeout(()=>{e.textContent="Copy"},1800)})}),s.appendChild(e)});
