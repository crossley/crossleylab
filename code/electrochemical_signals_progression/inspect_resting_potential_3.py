"""
Two-ion selective permeability model driven by a *negative charge wall* field.

This script simulates two ion species diffusing in a 2D box split into left and
right compartments by a central membrane (vertical wall) with two *type-selective*
channels. Unlike the point-attractor versions, the electrical bias here is a
uniform lateral field generated by a vertical “wall” of negative charge at a
fixed x-position (neg_wall_x). This produces primarily left–right drift (no
vertical electrical component).

Ion types / permeability proxy:
    - Type 0 (“Ion A”, orange): charge +1, can cross only through a narrow top
      channel (y in [30, 31]) → low permeability.
    - Type 1 (“Ion B”, green):  charge −1, can cross only through a wider bottom
      channel (y in [-40, -10]) → higher permeability.

Dynamics per timestep:
    1) Brownian motion: Gaussian noise in x and y.
    2) Electric drift toward/away from the negative wall:
         dx = neg_wall_x - x
         attract_x ∝ electric_strength * charge * dx / |dx|
       so + ions drift toward the negative wall and − ions drift away.
       (attract_y is set to 0, so the field is purely horizontal.)
    3) Mutual repulsion: pairwise repulsion (∝ 1 / r^2) scaled by
       repulsion_strength to reduce clustering/crowding.

Membrane / channel rule:
    - A particle “tries to cross” if its x-step would take it across the membrane
      boundary (left_wall or right_wall).
    - Crossing is allowed only if its y-position lies within its type’s channel.
    - Otherwise the x-position is reverted (blocked/reflected).

Initial condition:
    - Each ion type is split across sides (≈60% left, 40% right) and placed in a
      y-cluster near its own channel window to increase channel encounters.

Membrane potential proxy:
    - For each type, a compartment imbalance is tracked:
          V_A = (#Ion A on left) − (#Ion A on right)
          V_B = (#Ion B on left) − (#Ion B on right)
      and the total is V_total = V_A + V_B.
      This is a qualitative Δcharge readout, not a calibrated voltage.

Visual output:
    - Left panel: animated particle positions with selective channels shaded and
      the negative charge wall drawn as a dashed red vertical line.
    - Right panel: live plots of V_A, V_B, and V_total over time.
    - Saves 'goldman_with_neg_wall.mp4' using ffmpeg.

Conceptual focus:
    - Replacing a point attractor with a distributed “charged boundary” to create
      a more uniform lateral electrical bias.
    - Exploring how opposite charges + unequal permeability (channel geometry)
      produce distinct contributions to an emergent compartment imbalance.
"""
